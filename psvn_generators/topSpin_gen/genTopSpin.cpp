 ...
 * there are N different versions of the same state, so the state space is N times bigger than it needs to be.
 *
...
void topspin_basic_rule(int r) {
    ...
    if (r+K < N) { /* does not involve the last token (the reference token). Basic TopSpin rule. */
        ...
        for (int i=r+K; i<N-1; i++)  // the remaining positions are "don't care" in both LHS and RHS
        {
            tstr1=tstr1+"-  ";
            tstr2=tstr2+"-  ";
        }
...
/******* CREATE THE PSVN FILE ************/
    ...
    cout << "# This file was created by genTopSpin.cpp with the following command line parameters:\n#";
    for (int i=1; i<argc; ++i) {
        cout << "  " << argv[i] ;
    }
    string Nstring = convertInt(N-1);
